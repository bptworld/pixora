...
def _maybe_run_animation(options):
    ...
    team_key = (team.get("abbreviation") or favorite or "CBASE").upper()
    ...
    if previous is None:
        _RUN_STATE[key] = {"score": score, "animated": score, "seen": datetime.now(timezone.utc)}
        warm_priority_graphic(cache_key, lambda: _render_run_animation(animation_team))
        return None
    ...
    if score > last_score and score > animated:
        ...
        return {
            "body": cached_priority_graphic(cache_key, lambda: _render_run_animation(animation_team)),
            "dwell_secs": 4,
            "_stay": True,
            "_no_replay": True,
            "_group_wall": {"type": "run", "renderer": "_render_run_animation_frames", "team": dict(team), "dwell_secs": 6} if wall else None,
        }
    return None
...
def render(options=None):
    ...
    if animation:
        if animation.get("_group_wall"):
            normal_card = render_sport_card(opts, _URL, _CACHE, _COLOR, "NO CBASE")
            if normal_card:
