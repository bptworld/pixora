...
def _etsy_shop():
    image = Image.new("RGB", (64, 32), (8, 5, 2))
    draw = ImageDraw.Draw(image)
    orange = (245, 112, 35)
    muted = (180, 145, 120)
    white = (245, 250, 255)
    draw.rectangle((0, 0, 63, 8), fill=(26, 12, 4))
    draw_sharp_text(image, (2, -3), "ETSY", orange, BOLD)
    _center(image, "12", 6, white, BIG)
    _center(image, "OPEN", 23, muted, FONT)
    draw.rectangle((51, 3, 61, 13), outline=orange)
    draw.line((53, 3, 53, 0), fill=orange)
    draw.line((59, 3, 59, 0), fill=orange)
    return _webp(image)


def _launch_countdown():
    image = Image.new("RGB", (64, 32), (2, 6, 18))
    draw = ImageDraw.Draw(image)
    blue = (80, 175, 255)
    orange = (255, 160, 65)
    white = (238, 245, 255)
    muted = (135, 165, 190)
    draw.rectangle((0, 0, 63, 8), fill=(5, 14, 34))
    draw_sharp_text(image, (1, -3), "SPACEX", blue, BOLD)
    _center(image, "8H 7M", 10, white, BOLD, 0, 46)
    _center(image, "10:51PM", 16, orange, FONT, 0, 46)
    _center(image, "F9", 23, muted, FONT, 0, 46)
    x, y = 50, 5
    draw.polygon([(x + 4, y), (x + 8, y + 6), (x + 8, y + 18), (x, y + 18), (x, y + 6)], fill=(205, 220, 230))
    draw.polygon([(x + 4, y), (x + 1, y + 6), (x + 7, y + 6)], fill=orange)
...
def _crypto():
    ...
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, 63, 8), fill=(5, 18, 25))
    _center(image, "CRYPTO", -3, (255, 200, 80), BOLD)
    rows = [
        ("BTC", "$104K", (245, 165, 40)),
        ("ETH", "$3560", (170, 184, 205)),
        ("SOL", "$184", (125, 255, 205)),
    ]
    y = 8
    for coin, price, icon in rows:
        if coin == "BTC":
            draw.ellipse((1, y + 1, 7, y + 7), fill=icon, outline=(255, 220, 90))
        elif coin == "ETH":
            draw.polygon([(4, y), (8, y + 5), (4, y + 8), (0, y + 5)], fill=icon)
        else:
            draw.polygon([(1, y + 1), (8, y + 1), (6, y + 3), (0, y + 3)], fill=(125, 255, 205))
            draw.polygon([(1, y + 5), (8, y + 5), (6, y + 7), (0, y + 7)], fill=(180, 80, 255))
        draw_sharp_text(image, (10, y - 2), coin, (245, 250, 255), BOLD)
        w = draw.textbbox((0, 0), price, font=FONT)[2]
        draw_sharp_text(image, (63 - w, y - 2), price, (235, 245, 255), FONT)
        y += 8
    return _webp(image)
...
def _flights_overhead():
    image = Image.new("RGB", (64, 32), (0, 5, 18))
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, 63, 8), fill=(0, 15, 45))
    _center(image, "OVERHEAD", -3, (100, 190, 255), BOLD)
    # Tiny plane fallback, matching the card's no-logo behavior.
    draw.line((1, 13, 13, 9), fill=(110, 185, 255))
    draw.polygon([(6, 11), (2, 8), (9, 11)], fill=(170, 220, 255))
    draw.polygon([(8, 12), (5, 17), (11, 12)], fill=(110, 185, 255))
    draw_sharp_text(image, (16, 6), "WN3416", (245, 250, 255), BOLD)
    draw_sharp_text(image, (16, 14), "12K FT", (200, 230, 255), FONT)
    draw_sharp_text(image, (1, 22), "3MI NE", (150, 200, 255), FONT)
    return _webp(image)
...
def _airport_board():
    image = Image.new("RGB", (64, 32), (1, 6, 14))
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, 63, 8), fill=(12, 24, 36))
    draw_sharp_text(image, (1, -3), "BOS DEP", (100, 190, 255), FONT)
    draw_sharp_text(image, (1, 12), "7:10 DL2887 MCO", (235, 245, 255), FONT)
    return _webp(image)
...
CUSTOM = {
    ...
    "etsy_shop": _etsy_shop,
    "launch_countdown": _launch_countdown,
...
SAMPLE_OPTIONS = {
    "amc_now_playing": {"_demo": True},
    "cinemark_now_playing": {"_demo": True},
    "clock": {"zipCode": "02134"},
