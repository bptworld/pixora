...
static void main_draw_error_indicator(void) {
#if !CONFIG_BOARD_MATRIXPORTAL_S3_128X32
  draw_error_indicator_pixel();
#endif
}

static void build_interrupt_url(const char* image_url, char* out, size_t out_len) {
  if (!image_url || !out || out_len == 0) return;
  out[0] = '\0';
  const char* next = strstr(image_url, "/next");
  if (!next) {
    snprintf(out, out_len, "%s/interrupt", image_url);
    return;
  }
  size_t prefix_len = (size_t)(next - image_url);
  if (prefix_len > out_len - 12) prefix_len = out_len - 12;
  memcpy(out, image_url, prefix_len);
  out[prefix_len] = '\0';
  strlcat(out, "/interrupt", out_len);
}

static bool fetch_priority_interrupt(const char* interrupt_url,
                                     uint8_t** out_webp, size_t* out_len,
                                     uint8_t* brightness_pct,
                                     int32_t* dwell_secs,
                                     int32_t* start_delay_ms) {
  if (!interrupt_url || !interrupt_url[0] || !wifi_is_connected()) return false;
  if (message_is_quiet_hours()) return false;
  int status_code = 0;
  char* ota_url = NULL;
  int32_t interrupt_dwell = *dwell_secs;
  int32_t interrupt_start_delay = 0;
  uint8_t interrupt_brightness = *brightness_pct;
  uint8_t* interrupt_webp = NULL;
  size_t interrupt_len = 0;
  bool failed = remote_get(interrupt_url, &interrupt_webp, &interrupt_len,
                           &interrupt_brightness, &interrupt_dwell,
                           &status_code, &ota_url, &interrupt_start_delay);
  if (ota_url != NULL) free(ota_url);
  if (failed || status_code != 200 || interrupt_webp == NULL || interrupt_len == 0) {
    if (interrupt_webp) free(interrupt_webp);
    return false;
  }
  *out_webp = interrupt_webp;
  *out_len = interrupt_len;
  *brightness_pct = interrupt_brightness;
  *dwell_secs = interrupt_dwell;
  if (start_delay_ms != NULL) *start_delay_ms = interrupt_start_delay;
  return true;
}
            ...
            if (root) {
              ...
              if (cJSON_IsString(ota_item) && (ota_item->valuestring != NULL)) {
                ...
                if (ota_url) {
                  ...
                  ota_mark_running();
                  if (xTaskCreate(ota_task_entry, "ota_task", 8192, ota_url, 5,
                                  NULL) != pdPASS) {
...
void app_main(void) {
  ...
  ESP_LOGW(TAG, "App Main Start - Pixora %s reset=%s (%d)",
           FIRMWARE_VERSION, reset_reason_name(reset_reason), (int)reset_reason);
  ...
  if (button_boot) {
    ...
    while (!wifi_is_connected()) {
      ...
      counter++;
      if ((counter % 60) == 0) {
        ESP_LOGW(TAG, "Still waiting for Wi-Fi after %d seconds; not rebooting", counter);
      }
      vTaskDelay(pdMS_TO_TICKS(1 * 1000));
    }
  }

  if (sta_connected && nvs_get_ap_mode() && !button_boot) {
    ESP_LOGI(TAG, "WiFi is connected; closing setup server and enabling device endpoints");
    ap_stop();
    esp_wifi_set_mode(WIFI_MODE_STA);
    nvs_set_ap_mode(false);
    nvs_save_settings();
  } else if (nvs_get_ap_mode()) {
    ap_start_shutdown_timer();
  }

  // Start the push-message HTTP server (POST /message) and status endpoint.
  if (sta_connected) {
  ...
  while (true) {
    ...
    ESP_LOGW(TAG, "Pixora endpoint is not set. Waiting for configuration...");
  ...
  if (strncmp(image_url, "ws://", 5) == 0 ||
    ...
    char interrupt_url[256] = {0};
    build_interrupt_url(image_url, interrupt_url, sizeof(interrupt_url));
    ESP_LOGI(TAG, "Priority interrupt URL: %s", interrupt_url);
    bool was_quiet = false;
    for (;;) {
      // Keep polling during quiet hours so the server can render the clock-only
      // view and apply brightness or schedule changes made while quiet time is active.
      bool is_quiet = message_is_quiet_hours();
      if (is_quiet) {
        display_set_brightness(message_quiet_brightness());
        was_quiet = true;
      } else if (was_quiet) {
      ...
      // Push message from Hubitat (or any HTTP client) takes priority outside quiet time.
      if (!is_quiet && message_is_pending()) {
        message_display_and_wait();
      }

      uint8_t* webp;
      size_t len;
      static uint8_t brightness_pct = DISPLAY_DEFAULT_BRIGHTNESS;
      int32_t interrupt_dwell = app_dwell_secs;
      uint8_t interrupt_brightness = brightness_pct;
      int32_t interrupt_start_delay = 0;
      uint8_t* interrupt_webp = NULL;
      size_t interrupt_len = 0;
      if (!is_quiet &&
          fetch_priority_interrupt(interrupt_url, &interrupt_webp,
                                   &interrupt_len, &interrupt_brightness,
                                   &interrupt_dwell,
                                   &interrupt_start_delay)) {
        ESP_LOGI(TAG, "Priority interrupt preloaded before normal fetch (%d bytes), start delay %ld ms",
                 interrupt_len, (long)interrupt_start_delay);
        display_set_brightness(interrupt_brightness);
        isAnimating = -1;
        int priority_counter = gfx_update_at(interrupt_webp, interrupt_len,
                                             interrupt_dwell,
                                             interrupt_start_delay);
        interrupt_webp = NULL;
        int priority_timeout = 0;
        while (priority_counter >= 0 &&
               gfx_get_loaded_counter() != priority_counter &&
               priority_timeout < 20000) {
          vTaskDelay(pdMS_TO_TICKS(10));
          priority_timeout += 10;
        }
        while (isAnimating > 0 && !ota_is_running()) {
      ...
      if (ota_url != NULL) {
        ...
        ota_mark_running();
        if (xTaskCreate(ota_task_entry, "ota_task", 8192, ota_url, 5, NULL) !=
            pdPASS) {
          ESP_LOGE(TAG, "Failed to start OTA task");
          free(ota_url);
          ota_mark_idle();
          if (!fetch_failed && webp) {
            free(webp);
            webp = NULL;
          }
          continue;
      ...
      if (fetch_failed) {
        ...
        is_quiet = message_is_quiet_hours();
        if (is_quiet) {
          display_set_brightness(message_quiet_brightness());
          was_quiet = true;
        } else {
        ...
        int queued_counter = gfx_update_at(webp, len, app_dwell_secs,
                                           start_delay_ms);
        // Do not free(webp) here; ownership is transferred to gfx
        webp = NULL;
        bool priority_interrupted = false;
        ...
        if (isAnimating > 0) {
          ...
          int64_t display_wait_deadline_us =
              esp_timer_get_time() + 30000000LL;
          int64_t next_interrupt_poll_us = esp_timer_get_time() + 1000000LL;
          while (isAnimating > 0) {
            int64_t now_us = esp_timer_get_time();
            bool quiet_now = message_is_quiet_hours();
            if (quiet_now) {
              display_set_brightness(message_quiet_brightness());
              is_quiet = true;
              was_quiet = true;
            ...
            if (now_us >= display_wait_deadline_us) {
              ESP_LOGE(TAG,
                       "Timed out waiting for current webp; forcing queued "
                       "image to load");
              isAnimating = -1;
              break;
            }
            if (!quiet_now && now_us >= next_interrupt_poll_us && !ota_is_running()) {
              next_interrupt_poll_us = now_us + 1000000LL;
              uint8_t* interrupt_webp = NULL;
              size_t interrupt_len = 0;
              uint8_t interrupt_brightness = brightness_pct;
              int32_t interrupt_dwell = app_dwell_secs;
              int32_t interrupt_start_delay = 0;
              if (fetch_priority_interrupt(interrupt_url, &interrupt_webp,
                                           &interrupt_len,
                                           &interrupt_brightness,
                                           &interrupt_dwell,
                                           &interrupt_start_delay)) {
                ESP_LOGI(TAG, "Priority interrupt received (%d bytes), start delay %ld ms",
                         interrupt_len, (long)interrupt_start_delay);
                display_set_brightness(interrupt_brightness);
                isAnimating = -1;
                priority_counter = gfx_update_at(interrupt_webp, interrupt_len,
                                                 interrupt_dwell,
                                                 interrupt_start_delay);
                priority_interrupted = true;
                interrupt_webp = NULL;
                break;
              }
            }
        ...
        // queued_counter);
        int timeout = 0;
        int wait_counter = priority_interrupted ? priority_counter : queued_counter;
        while (wait_counter >= 0 && gfx_get_loaded_counter() != wait_counter &&
               timeout < 20000) {
          vTaskDelay(pdMS_TO_TICKS(10));
          timeout += 10;
        }
        if (timeout >= 20000) {
          ...
          ESP_LOGI(TAG, "Gfx task loaded image counter %d ms", wait_counter);
        }

        int64_t next_post_load_interrupt_poll_us =
        ...
        while (isAnimating > 0 && !ota_is_running()) {
          ...
          if (!quiet_now && now_us >= next_post_load_interrupt_poll_us) {
            next_post_load_interrupt_poll_us = now_us + 1000000LL;
            uint8_t* interrupt_webp = NULL;
            size_t interrupt_len = 0;
            uint8_t interrupt_brightness = brightness_pct;
            int32_t interrupt_dwell = app_dwell_secs;
            ...
            if (fetch_priority_interrupt(interrupt_url, &interrupt_webp,
                                         ...
                                         &interrupt_start_delay)) {
              ESP_LOGI(TAG,
                       "Priority interrupt received while displayed webp was "
                       "running (%d bytes), start delay %ld ms",
                       interrupt_len, (long)interrupt_start_delay);
              display_set_brightness(interrupt_brightness);
              isAnimating = -1;
              int post_priority_counter = gfx_update_at(
                  interrupt_webp, interrupt_len, interrupt_dwell,
                  interrupt_start_delay);
              interrupt_webp = NULL;
              int post_priority_timeout = 0;
              while (post_priority_counter >= 0 &&
                     gfx_get_loaded_counter() != post_priority_counter &&
                     post_priority_timeout < 20000) {
                vTaskDelay(pdMS_TO_TICKS(10));
                post_priority_timeout += 10;
              }
              next_post_load_interrupt_poll_us =
                  esp_timer_get_time() + 1000000LL;
            }
          }
          vTaskDelay(pdMS_TO_TICKS(25));
        ...
        // gfx_loop owns isAnimating so the fetch loop stays tied to the
        // display task instead of racing it.
      }
