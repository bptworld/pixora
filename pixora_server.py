...
def infer_device_target(device):
    if not isinstance(device, dict):
        return ""
    target = str(device.get("target") or "").strip()
    if target:
        return target
    text = " ".join(
    ...
    def do_POST(self):
        ...
        if self.path == "/api/test-card-animation":
            try:
                ...
                if mode in ("group", "group_wall", "wall") or group_id:
                    ...
                    frame_renderer = "_render_run_animation_frames" if kind == "run" else "_render_goal_animation_frames"
                    queued_wall = queue_sports_moment_wall(group, module, frame_renderer, team, dwell_secs=6, source="test", label=f"{card_id}-{kind}") if group else None
                    if not queued_wall:
                ...
                NEXT_STATE[device_id] = {
                    ...
                    "custom_dwell": 6,
                    "frames": [{"body": body, "dwell_secs": 6, "no_replay": True}],
                }
            ...
            try:
                ...
                if mode in ("group", "group_wall", "wall") or group_id:
                    ...
                    if not group:
                        ...
                        return
                    queued_wall = queue_mlb_run_wall(group, mlb, team, dwell_secs=6, source="test")
                    if not queued_wall:
                        self._send_json(400, {"ok": False, "error": "No devices in this group"})
                        return
                    log(f"[test] queued MLB run wall animation for {group.get('id')} ({favorite})")
                    self._send_json(200, {"ok": True, "queued": True, "mode": "wall", "device": device_id, "group": group.get("id"), "team": favorite, **queued_wall})
                    return

                team["_width"] = device_width(device)
                body = mlb._render_run_animation(team)
                current = NEXT_STATE.get(device_id, {})
                NEXT_STATE[device_id] = {
                    "index": current.get("index", 0),
                    "until": datetime.min.replace(tzinfo=timezone.utc),
                ...
                log(f"[test] queued MLB run animation for {device_id} ({favorite})")
                self._send_json(200, {"ok": True, "queued": True, "device": device_id, "team": favorite})
            except Exception as error:
                self._send_json(500, {"ok": False, "error": str(error)})
            return
        ...
        if self.path == "/api/test-nhl-goal":
            try:
                ...
                if mode in ("group", "group_wall", "wall") or group_id:
                    group = group_for_id(group_id) if group_id else group_for_device_id(device_id)
                    queued_wall = queue_sports_moment_wall(group, nhl, "_render_goal_animation_frames", team, dwell_secs=6, source="test", label="nhl-goal") if group else None
                    if not queued_wall:
                        self._send_json(404, {"ok": False, "error": "Group not found or has no devices"})
                        return
                    log(f"[test] queued NHL goal wall animation for {group.get('id')} ({favorite})")
                    self._send_json(200, {"ok": True, "queued": True, "mode": "wall", "device": device_id, "group": group.get("id"), "team": favorite, **queued_wall})
                    return

                team["_width"] = device_width(device)
                body = nhl._render_goal_animation(team)
                current = NEXT_STATE.get(device_id, {})
                NEXT_STATE[device_id] = {
                    "index": current.get("index", 0),
                    "until": datetime.min.replace(tzinfo=timezone.utc),
                ...
                log(f"[test] queued NHL goal animation for {device_id} ({favorite})")
                self._send_json(200, {"ok": True, "queued": True, "device": device_id, "team": favorite})
            except Exception as error:
                self._send_json(500, {"ok": False, "error": str(error)})
            return
        ...
        if self.path == "/api/test-soccer-goal":
            try:
                ...
                if mode in ("group", "group_wall", "wall") or group_id:
                    group = group_for_id(group_id) if group_id else group_for_device_id(device_id)
                    queued_wall = queue_sports_moment_wall(group, soccer, "_render_goal_animation_frames", team, dwell_secs=6, source="test", label="soccer-goal") if group else None
                    if not queued_wall:
                        self._send_json(404, {"ok": False, "error": "Group not found or has no devices"})
                        return
                    log(f"[test] queued soccer goal wall animation for {group.get('id')} ({favorite}, {league})")
                    self._send_json(200, {"ok": True, "queued": True, "mode": "wall", "device": device_id, "group": group.get("id"), "team": favorite, "league": league, **queued_wall})
                    return

                team["_width"] = device_width(device)
                body = soccer._render_goal_animation(team)
                current = NEXT_STATE.get(device_id, {})
                NEXT_STATE[device_id] = {
                    "index": current.get("index", 0),
                    "until": datetime.min.replace(tzinfo=timezone.utc),
                ...
                log(f"[test] queued soccer goal animation for {device_id} ({favorite}, {league})")
                self._send_json(200, {"ok": True, "queued": True, "device": device_id, "team": favorite, "league": league})
            except Exception as error:
                self._send_json(500, {"ok": False, "error": str(error)})
            return
        ...
        if self.path == "/api/test-nfl-score":
            try:
                ...
                if kind not in ("touchdown", "field_goal", "safety", "score"):
                    kind = "touchdown"
                ...
                if mode in ("group", "group_wall", "wall") or group_id:
                    group = group_for_id(group_id) if group_id else group_for_device_id(device_id)
                    queued_wall = queue_sports_moment_wall(group, nfl, "_render_score_animation_frames", team, kind=kind, dwell_secs=6, source="test", label=f"nfl-{kind}") if group else None
                    if not queued_wall:
                        self._send_json(404, {"ok": False, "error": "Group not found or has no devices"})
                        return
        ...
        if self.path == "/api/test-ufl-score":
            try:
                ...
                if mode in ("group", "group_wall", "wall") or group_id:
                    group = group_for_id(group_id) if group_id else group_for_device_id(device_id)
                    queued_wall = queue_sports_moment_wall(group, ufl, "_render_score_animation_frames", team, kind=kind, dwell_secs=6, source="test", label=f"ufl-{kind}") if group else None
                    if not queued_wall:
                        self._send_json(404, {"ok": False, "error": "Group not found or has no devices"})
                        return
        ...
        if self.path == "/api/settings/save":
            try:
                ...
                if "registryUrl" in payload:
                    settings["registryUrl"] = str(payload.get("registryUrl") or "").strip()
            ...
            try:
                ...
                device = {
                    ...
                    **existing,
                    ...
                    "server":   re.sub(r"/[^/]+/next/?$", "", remote_url),
                    "endpoint": remote_url,
                    "target":   existing.get("target") or payload.get("target") or "matrixportal-s3-waveshare",
                    "cards":    existing.get("cards") or ["clock"],
                    "createdAt": existing.get("createdAt") or datetime.now(timezone.utc).isoformat(),
                }
                save_device(device)
